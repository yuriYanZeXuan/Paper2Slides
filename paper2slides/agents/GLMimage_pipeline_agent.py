"""GLM-Image agent pipeline entry.

1. 复用 Paper2Slides 的主 RAG / summary / plan / generate 流水线
2. 固定使用 GLM-Image 本地后端生成 poster 或 slides
3. 对生成的图片（poster 或每一张 slide）调用 PosterRefinerGLMAgent 做一次小字强化
...
import paper2slides.agents.tools.poster_text_score  # noqa: F401
import paper2slides.agents.tools.poster_text_match  # noqa: F401
import paper2slides.agents.tools.glm_patch_edit  # noqa: F401
from paper2slides.agents.tools.config_loader import set_config_path
from paper2slides.agents.poster_refiner_glm import PosterRefinerGLMAgent
...
def run_glm_image_agent_pipeline(args: argparse.Namespace) -> None:
    agent = "glm_image_pipeline_agent"
    ...
    config = {
        ...
        "image_backend": "glm_image",  # 固定使用本地 GLM-Image
    ...
    glm_model = args.local_image_model or "THUDM/GLM-Image-1.0"
    refiner = PosterRefinerGLMAgent(
    ...
    for img_path in image_paths:
        log_agent_info(agent, f"refine image (GLM patch mode): {img_path.name}")
        img = Image.open(img_path).convert("RGB")
        refined = refiner.run(
            image=img,
            clarity_threshold=9.0,
            max_rounds=3,
            bbox_limit=5,
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Paper2Slides GLM-Image Agent Pipeline",
    ...
    parser.add_argument("--local-image-model", default="THUDM/GLM-Image-1.0",
                        help="Local GLM-Image model path or repo id")
    parser.add_argument("--device", default="cuda",
                        help="Device for local GLM-Image (cuda/cpu)")
    ...
    run_glm_image_agent_pipeline(args)
